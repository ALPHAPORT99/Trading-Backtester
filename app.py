...
strategy = st.sidebar.selectbox("Select Strategy", [
    ...
    "Triple Moving Average Crossover", "VWAP", "Rate of Change (ROC)", 
    "Elder Ray Index", "Mass Index", "Chande Momentum Oscillator (CMO)", 
    "Guppy Multiple Moving Averages (GMMA)", "Ultimate Oscillator", "Force Index",
    "Ease of Movement (EOM)", "Chaikin Money Flow (CMF)"
])
...
if run_backtest:
    ...
    cash = capital  # Keep track of cash balance
    shares = 0  # Keep track of shares owned

    # **Backtesting Logic for Strategies**
    if strategy == "Simple Moving Average Crossover":
        ...
        for i in range(1, len(df)):
            if df['SMA_50'].iloc[i] > df['SMA_200'].iloc[i] and df['SMA_50'].iloc[i - 1] <= df['SMA_200'].iloc[i - 1]:
                if cash >= df['Close'].iloc[i]:  # Buy condition
        ...
        for i in range(1, len(df)):
            if df['RSI'].iloc[i] < 30:
                if cash >= df['Close'].iloc[i]:  # Buy when RSI < 30
                    shares = cash // df['Close'].iloc[i]
                    cash -= shares * df['Close'].iloc[i]
                    buy_signals.append(df.index[i])
            elif df['RSI'].iloc[i] > 70:
                if shares > 0:  # Sell when RSI > 70
                    cash += shares * df['Close'].iloc[i]
                    shares = 0
                    sell_signals.append(df.index[i])
